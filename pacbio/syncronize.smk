...
# (exclude_donor: [TSP31, ...]) or a single/comma-separated string from the CLI
# (--config exclude_donor=TSP31,TSP32).
...
def _tissues_for_donors(donors):
    ...
    config['sample_metadata_csv'] (defaults to csvs/sample_metadata.csv,
    the same source phasing.smk uses)."""
    donors = set(donors)
...
rule all:
    input:
        ...
        # Optional --mono-exon-filtered Seurat (config['mono_exon_seurat']; empty list otherwise)
        (expand(os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/genes_seurat/matrix.mtx"), tissue=TISSUES) if config.get("mono_exon_seurat", False) else []),
        (expand(os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/isoforms_seurat/matrix.mtx"), tissue=TISSUES) if config.get("mono_exon_seurat", False) else [])
...
rule pigeon_filter_recollapsed_monoexon:
    # Optional branch (config['mono_exon_seurat']): pigeon filter WITH --mono-exon, which
    # DROPS mono-exonic transcripts. Runs in a dedicated classify_monoexon/ dir (copies the
    # classification + junctions + sorted gff there) so pigeon's input-derived output names
    # don't overwrite the default mono-exon-KEPT filtered outputs in classify/.
    input:
        classification = os.path.join(WORKDIR, RECOLLAPSED + "/classify/{tissue}/collapse_classification.txt"),
        junctions = os.path.join(WORKDIR, RECOLLAPSED + "/classify/{tissue}/collapse_junctions.txt"),
        sorted_gff = os.path.join(WORKDIR, RECOLLAPSED + "/collapse/{tissue}/collapse.recollapsed.gff.sorted")
    output:
        classification = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse_classification.txt"),
        junctions = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse_junctions.txt"),
        sorted_gff = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse.recollapsed.sorted.gff"),
        filtered_gff = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse.recollapsed.sorted.filtered_lite.gff"),
        filtered_classification = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse_classification.filtered_lite_classification.txt"),
        filtered_junctions = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse_classification.filtered_lite_junctions.txt")
    params:
        outdir = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}")
    log:
        os.path.join(WORKDIR, "logs/" + RECOLLAPSED + "/{tissue}_filter_monoexon.log")
    threads: get_resource("pigeon_filter", "threads", DEFAULT_THREADS)
    resources:
        mem=get_resource("pigeon_filter", "mem", DEFAULT_MEMORY),
        partition=get_resource("pigeon_filter", "partition", DEFAULT_PARTITION),
    ...
    shell:
        """
        mkdir -p {params.outdir}
        cp {input.classification} {output.classification}
        cp {input.junctions} {output.junctions}
        cp {input.sorted_gff} {output.sorted_gff}
        pigeon filter \
            {output.classification} \
            --isoforms {output.sorted_gff} \
            --mono-exon \
            -j {threads} \
            2> {log}
        """
...
rule pigeon_make_seurat_recollapsed_monoexon:
    # Seurat matrices from the --mono-exon-filtered classification, in a separate
    # seurat_monoexon/ dir (same options as pigeon_make_seurat_recollapsed).
    input:
        classification = os.path.join(WORKDIR, RECOLLAPSED + "/classify_monoexon/{tissue}/collapse_classification.filtered_lite_classification.txt"),
        dedup = os.path.join(WORKDIR, "isoseq/dedup/{tissue}/{tissue}.dedup.fasta"),
        group = os.path.join(WORKDIR, RECOLLAPSED + "/collapse/{tissue}/collapse.recollapsed.group.txt")
    output:
        gene_matrix = os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/genes_seurat/matrix.mtx"),
        isoform_matrix = os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/isoforms_seurat/matrix.mtx")
    params:
        outdir = os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}"),
        keep_novel = "--keep-novel-genes" if config.get("keep_novel_genes", False) else "",
        keep_ribo_mito = "--keep-ribo-mito-genes" if config.get("keep_ribo_mito_genes", False) else ""
    threads: get_resource("pigeon_make_seurat", "threads", DEFAULT_THREADS)
    resources:
        ...
        slurm_extra=get_resource("pigeon_make_seurat", "slurm_extra", DEFAULT_SLURM_EXTRA)
    log:
        os.path.join(WORKDIR, "logs/" + RECOLLAPSED + "/{tissue}_make_seurat_monoexon.log")
    shell:
        """
        mkdir -p {params.outdir}
        pigeon make-seurat \
...
rule all_recollapsed:
    input:
        ...
        # Optional --mono-exon-filtered Seurat (enabled by config['mono_exon_seurat']; empty otherwise)
        seurat_monoexon_gene_outputs = expand(os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/genes_seurat/matrix.mtx"), tissue=TISSUES) if config.get("mono_exon_seurat", False) else [],
        seurat_monoexon_isoform_outputs = expand(os.path.join(WORKDIR, RECOLLAPSED + "/seurat_monoexon/{tissue}/isoforms_seurat/matrix.mtx"), tissue=TISSUES) if config.get("mono_exon_seurat", False) else []

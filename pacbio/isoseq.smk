...
# Per-barcode subsampling is OFF by default in the pacbio pipeline (groupdedup
# runs on the full sorted BAM). Only tissues listed in config['subsample_tissues']
# take the subsample path (cap = max_reads_per_barcode); every other tissue keeps
# running dedup on the full sorted BAM exactly as before.
SUBSAMPLE_TISSUES = set(config.get("subsample_tissues", []) or [])

def groupdedup_bam(wildcards):
    """Input BAM for isoseq_groupdedup: subsampled BAM for the selected tissues,
    otherwise the full CB-sorted BAM."""
    t = wildcards.tissue
    if t in SUBSAMPLE_TISSUES:
        return os.path.join(WORKDIR, f"isoseq/subsample/{t}/{t}.fltnc.corrected.sorted.subsampled.bam")
    return os.path.join(WORKDIR, f"isoseq/sort/{t}/{t}.fltnc.corrected.sorted.bam")

def groupdedup_pbi(wildcards):
    """.pbi for the groupdedup input BAM. For the full-sort path it is ancient()
    so rebuilding the index never retriggers dedup on already-processed tissues;
    for the subsample path the .pbi is produced by subsample_cell_barcode and is a
    normal dependency."""
    p = groupdedup_bam(wildcards) + ".pbi"
    return p if wildcards.tissue in SUBSAMPLE_TISSUES else ancient(p)
...
rule subsample_cell_barcode:
    # Cap reads per cell barcode (config['max_reads_per_barcode']) before dedup.
    # Only runs for tissues in config['subsample_tissues'] (selected via
    # groupdedup_bam); all other tissues dedup the full sorted BAM directly.
    input:
        sorted_bam = os.path.join(WORKDIR, "isoseq/sort/{tissue}/{tissue}.fltnc.corrected.sorted.bam")
    output:
...
rule isoseq_groupdedup:
    input:
        # groupdedup runs on the full CB-sorted BAM by default (no per-barcode
        # cap). Tissues listed in config['subsample_tissues'] instead dedup the
        # subsampled BAM from subsample_cell_barcode — see groupdedup_bam/_pbi.
        # For the full-sort path the .pbi is ancient() so rebuilding the index
        # never retriggers dedup on already-processed tissues.
        sorted_bam = groupdedup_bam,
        sorted_pbi = groupdedup_pbi
    output:
